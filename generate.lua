...
--
-- Usage:
--     GLUON_ROOT=path/to/gluon lua generate.lua OUTDIR
--
-- GLUON_ROOT points at a Gluon source checkout (default "gluon", the submodule
-- in this repo). All values a real router would read from uci/site live in the
-- MOCK tables below; tweak them to exercise different states.

-- Gluon source checkout to render against (submodule by default).
local GLUON = (os.getenv("GLUON_ROOT") or "gluon"):gsub("/+$", "")

-- Make the in-tree Lua libraries requirable.
package.path = table.concat({
	GLUON .. "/package/gluon-web-model/luasrc/usr/lib/lua/?.lua",
	GLUON .. "/package/gluon-web/luasrc/usr/lib/lua/?.lua",
	package.path,
}, ";")
...
local function discover(kind, ext)
	...
	local files = popen_lines(string.format(
		"find %s/package -path '*/luasrc/lib/gluon/config-mode/%s/*.%s' 2>/dev/null", GLUON, kind, ext))
	if kind == "view" then
		files = popen_lines(string.format(
			"find %s/package -path '*/files/lib/gluon/config-mode/%s/*.%s' 2>/dev/null", GLUON, kind, ext))
	end
...
local SECTION_FILES = (function()
	local files = popen_lines(string.format(
		"find %s/package -path '*/luasrc/lib/gluon/config-mode/wizard/*.lua' 2>/dev/null", GLUON))
	table.sort(files, function(a, b) return basename(a) < basename(b) end)
	return files
end)()
local CONTROLLER_FILES = (function()
	-- The dispatcher globs base/*.lua then base/*/*.lua (base being the single
	-- merged controller dir on the device). Mirror that order by sorting on the
	-- path *below* controller/: top-level files first, then one dir deeper,
	-- each group alphabetically.
	local files = popen_lines(string.format(
		"find %s/package -path '*/luasrc/lib/gluon/config-mode/controller/*.lua' 2>/dev/null", GLUON))
	local function key(f) return f:match("/controller/(.+)$") or f end
	table.sort(files, function(a, b)
		local ka, kb = key(a), key(b)
		local da = ka:find("/") and 1 or 0
		local db = kb:find("/") and 1 or 0
...
local function emit_multilist_input(o)
	...
	for i, e in ipairs(entries) do
		w(string.format("<label%s%s>", attr("data-index", i), attr("data-depends", o:deplist(e.deps))))
		w("<input data-update=\"click change\" type=\"checkbox\"")
		w(attr("id", id .. "." .. e.key) .. attr("name", id) .. attr("value", e.key))
...
local function emit_welcome(s)  -- the wizard/welcome custom template
	w(string.format("<fieldset class=\"gluon-section\" id=\"%s\" data-index=\"%s\">", s:id(), s.index))
	w("<p>" .. esc(MOCK.welcome) .. "</p></fieldset>")
end

local function emit_unknown_section(s)  -- any other custom section template
	w(string.format("<fieldset class=\"gluon-section\" id=\"%s\" data-index=\"%s\">", s:id(), s.index))
	w(string.format("<em>[custom section template: %s]</em></fieldset>", esc(s.template or "?")))
end
...
local function emit_form(f)
	...
	for _, child in ipairs(f.children) do
		if child.template == "model/section" then
			emit_section(child)
		elseif child.template == "wizard/welcome" then
			emit_welcome(child)
		else
			emit_unknown_section(child)
		end
	end
	w("</div>")
...
for _, page in ipairs(pages) do
	...
	if seen[fname] then
		io.stderr:write(string.format("warning: filename collision for %s (overwriting)\n", fname))
	end
	seen[fname] = true
